...
class AdeptController extends Controller
{
    ...
    public function index(){
        ...
        if ($adepts === null) {
            ...
            return view('Adept.index')->with('message','add details first');
        }else{
        $profileBrief= DB::table('adepts')
                        ->where('user_id', Auth::id())
                        ->select('adepts.first_name','adepts.last_name', 'adepts.Phone', 'adepts.address', 'adepts.city', 'adepts.email','adepts.date_of_birth')
    ...
    public function viewProfile(){
        ...
        $allCourses = DB::table('courses')
                        ->where('user_id', Auth::id())
                        ->where('status', 1)
                        ->select('courses.name','courses.category','courses.level','courses.year','courses.obtained')
                        ->get();
    ...
    public function addDetails(Request $request)
     {
        ...
        $adepts = Adept::where('user_id', '=',  Auth::id())->first();
        if ($adepts != null) {
            return redirect('/adept')->with('message','you have already added your profile details or maybe you wouls like to edit');
        }else{
    ...
    public function search(Request $request){
        ...
        if($query === null){
            return redirect()->back()->with('message','you have not entered any category !!!!');
        }else{
        $data = DB::table('courses')
        ->where('category', 'like', '%'.$query.'%')
        ->where('status', 1)
        ->select('courses.obtained', 'courses.name','courses.category', 'courses.level')
        ->get();

        $numberOfResults = DB::table('courses')
        ->where('category', 'like', '%'.$query.'%')
        ->where('status', 1)
        ->select('courses.obtained', 'courses.name','courses.category', 'courses.level')
        ->count();
        ...
        if(count($data) > 0){
            return view('Adept.searchResults')->withDetails($data)->with($data1)->withQuery ( $query);}
            else{
             return redirect()->back()->with('message','sorry ,no courses done maching the category');
            }     
        }
    }
}
